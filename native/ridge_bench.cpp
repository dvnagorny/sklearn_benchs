...
{
    dar::training::Batch<> algorithm;
    algorithm.input.set(dar::training::data, makeTable(X, rows, cols));
    algorithm.input.set(dar::training::dependentVariables, makeTable(Y, rows, cols));
    algorithm.compute();
...
{
    dar::prediction::Batch<> algorithm;
    algorithm.input.set(dar::prediction::data, makeTable(X, rows, cols));
    algorithm.input.set(dar::prediction::model, trainingResult->get(dar::training::model));
    algorithm.compute();
    algorithm.getResult()->get(dar::prediction::prediction);
}
